...
Actions taken using sudo are logged, making it easier to track who performed a certain action on the system.<br /><br />
    ...
    * Configuration `sudo nano /etc/sudoers.d/sudo_config`
   ...
   Defaults  iolog_dir="/var/log/sudo/sudo.log"
   Defaults  requiretty
   Defaults  secure_path="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/snap/bin"
   ```

6. **Users and groups manipulation**<br /><br />
In addition to the root user, a user with my login as username must be present and included in `user42` and `sudo` groups.<br /><br />
...
7. **SSH**<br /><br />
SSH (Secure Shell) is a cryptographic network protocol used for securely accessing and managing computers over an unsecured network. SSH is commonly used for remote login to servers, allowing administrators and users to interact with a machine from a distance.<br /><br />
    * Installation
    ```bash
    ...
    * Configuration
      - `sudo nano /etc/ssh/ssh_config` (SSH server): change `Port 4242` to allow SSH service to run on port 4242
      - `sudo nano /etc/ssh/sshd_config` (SSH client): change `Port 4242` to allow SSH service to run on port 4242 only and `PermitRootLogin no` to block SSH connection as root
...
It helps secure a system by controlling incoming and outgoing network traffic based on predefined rules. By default, UFW denies all incoming connections and allows all outgoing connections, which is a secure starting point.<br /><br />
...
9. **Strong password policy**<br /><br />
    * Password aging
      - Password has to expire every 30 days
      - The minimum number of days allowed before the modification of a password will be set to 2
      - The user has to receive a warning message 7 days before their password expires

    * Configuration `nano /etc/login.defs`
   <!-- image --> <br />
   ...
   * Check passowrd status for a \<user\>
    ...
    * Configuration `nano /etc/pam.d/common-password`
    ```
...
Develop a script in bash that will display the following information in all terminals every 10 minutes:
    ...
    * The number of commands executed with the sudo program
    <br />
    My script can be viewed in the file `monitoring.sh` <br /><br />
...
11. **Lighttpd**<br /><br />
Lighttpd is a lightweight, high-performance web server designed to efficiently serve web content with minimal resource usage. It is particularly well-suited for systems with limited hardware resources or environments requiring high-speed and low-memory web server solutions.<br /><br />
...
15. **Wordpress configuration**<br /><br />
